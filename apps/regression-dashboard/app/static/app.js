...
const isTechnicalError=r=>r.state==='error'||r.auto_evaluation?.technical_error===true;
const displayedStatus=r=>isTechnicalError(r)?'TECHNICAL_ERROR':effective(r);
const displayedStatusLabel=r=>isTechnicalError(r)?'ТЕХНИЧЕСКАЯ ОШИБКА':effective(r);
...
function renderDashboard(){
  ...
  const order={TECHNICAL_ERROR:0,CRITICAL:1,FAIL:2,PARTIAL:3,PASS:4,UNREVIEWED:5};
  const sorted=[...r.results].filter(x=>x.state!=='pending').sort((a,b)=>order[displayedStatus(a)]-order[displayedStatus(b)]).slice(0,6);
  $('#recentResults').innerHTML=sorted.length?sorted.map(resultRow).join(''):'<p class="threshold">Ответов пока нет.</p>';
}
function resultRow(r){return `<div class="result-row" onclick="openDetail(${r.id})"><span class="case-id">${esc(r.case_id)}</span><span class="${statusClass(displayedStatus(r))}">${displayedStatusLabel(r)}</span><span class="result-question">${esc(r.question)}</span><span class="result-latency">${fmtMs(r.latency_ms)}</span><span>›</span></div>`}
...
function filterResults(){
  ...
  const rows=state.run.results.filter(r=>(!q||`${r.case_id} ${r.question} ${r.answer||''} ${r.technical_error||''}`.toLowerCase().includes(q))&&(!s||displayedStatus(r)===s)&&(!c||r.category===c));
  $('#resultCount').textContent=`${rows.length} результатов`;$('#resultsTable').innerHTML=`<table class="data-table"><thead><tr><th>Кейс</th><th>LLM</th><th>Алгоритм</th><th>Итог</th><th>Ответ</th><th>Категория</th><th>Время</th></tr></thead><tbody>${rows.map(r=>{const technical=isTechnicalError(r);return `<tr onclick="openDetail(${r.id})"><td class="case-id">${esc(r.case_id)}</td><td><span class="${statusClass(technical?'UNREVIEWED':llmStatus(r))}">${technical?'—':llmStatus(r)}</span></td><td><span class="${statusClass(technical?'UNREVIEWED':algorithmicStatus(r))}">${technical||algorithmicStatus(r)==='UNREVIEWED'?'—':algorithmicStatus(r)}</span></td><td><span class="${statusClass(displayedStatus(r))}">${displayedStatusLabel(r)}${r.manual_status?' · ручн.':''}</span></td><td class="answer-preview">${esc(technical?`Техническая ошибка: ${r.technical_error||'ответ агента не завершён'}`:(r.answer||r.state))}</td><td>${esc(r.category)}</td><td>${fmtMs(r.latency_ms)}</td></tr>`}).join('')}</tbody></table>`;
}
function boolLabel(v){return v==null?'—':v?'Да':'Нет'}
function openDetail(id){
  const r=state.run.results.find(x=>x.id===id);if(!r)return;const e=r.auto_evaluation||{},a=e.algorithmic,llm=llmStatus(r),technical=isTechnicalError(r);const failedChecks=(a?.checks||[]).filter(x=>['FAIL','SKIP'].includes(x.status));
  ...
  const algorithmicBlock=technical?`<div class="drawer-section"><h3>Алгоритмическая проверка</h3><div class="text-block muted-block">Не выполнялась: ответ агента не был завершён.</div></div>`:a?`<div class="drawer-section"><h3>Алгоритмическая проверка</h3>${reviewWarning}<div class="text-block"><span class="${statusClass(a.status)}">${esc(a.status)}</span><p class="evaluation-reason">${esc(a.reason||'Причина не указана')}</p></div>${failedChecks.length?`<ul class="check-list">${failedChecks.map(c=>`<li><span class="${statusClass(c.status==='SKIP'?'UNREVIEWED':'FAIL')}">${esc(c.status)}</span><div><b>${esc(c.label)}</b><small>${esc(c.details)}</small></div></li>`).join('')}</ul>`:'<p class="all-checks-pass">Все применимые алгоритмические проверки пройдены.</p>'}</div>`:`<div class="drawer-section"><h3>Алгоритмическая проверка</h3><div class="text-block muted-block">Для этой старой записи алгоритмические данные отсутствуют.</div></div>`;
  const llmBlock=technical?`<div class="drawer-section"><h3>Оценка качества</h3><div class="text-block muted-block">Не выполнялась как обычная оценка качества: зафиксирована техническая ошибка.</div></div>`:`<div class="drawer-section"><h3>Вердикт LLM-судьи</h3><div class="text-block"><span class="${statusClass(llm)}">${esc(llm==='UNREVIEWED'?'—':llm)}</span><p class="evaluation-reason">${esc(e.reason||'Оценка ещё не выполнена')}</p></div><div class="eval-grid">${[['Факты',boolLabel(e.factual_correct)],['Полезность',boolLabel(e.useful)],['Ложный отказ',boolLabel(e.false_refusal)],['Вердикт пользователю',boolLabel(e.verdict_given)],['Web Search',boolLabel(e.web_search_used)],['Маркировка Web',boolLabel(e.web_search_labeled_correctly)]].map(x=>`<div class="eval-item"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('')}</div></div>`;
  const finalLabel=r.manual_status?`Ручной итог · авто ${r.auto_status||'—'}`:'Итоговый статус';
  $('#drawerContent').innerHTML=`<p class="eyebrow">${esc(r.case_id)} · ${esc(r.category)}</p><h2>${esc(r.question)}</h2>${technical?`<div class="technical-error-flag">Техническая ошибка: ${esc(r.technical_error||'ответ агента не завершён')}. Кейс исключён из обычных метрик качества.</div>`:''}<div class="verdict-grid"><div><span>LLM-судья</span><b class="${statusClass(technical?'UNREVIEWED':llm)}">${esc(technical?'—':llm==='UNREVIEWED'?'—':llm)}</b></div><div><span>Алгоритм</span><b class="${statusClass(technical?'UNREVIEWED':a?.status)}">${esc(technical?'—':a?.status||'—')}</b></div><div><span>${esc(finalLabel)}</span><b class="${statusClass(displayedStatus(r))}">${esc(displayedStatusLabel(r))}</b></div></div>
  <div class="drawer-section"><h3>Ответ агента</h3><div class="text-block">${esc(r.answer||(technical?'Ответ не получен из-за технической ошибки':'Ответ ещё не получен'))}</div></div>
  ${llmBlock}
  ${algorithmicBlock}
